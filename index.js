...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            ...
            + 'FN: Ardyy Ganss\n' // NAMA KONTAK OWNER
            + 'ORG:Owner BOT;\n' // NAMA CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=6281344309802:+62 813-4430-9802\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '.'
blocked = ['@s.whatsapp.net']
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
			  levelon: '*[SUCCESS]* Level Telah Di Aktifkan✔️\n*ONIICHAN!!*',
			  leveloff: '*[SUCCESS]* Level Telah Di Nonaktifkan✔️\n*ONIICHAN!!*',
				levelnoton: 'Level Sedang Tidak Aktif\n*BAKA!!* ',
				levelnol: 'Level Lu Masih Nol *BAKA!!*',
			...
			const ownerNumber = ["6281344309802@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6281344309802@s.whatsapp.net"]
			const adminbotnumber = ["6281344309802@s.whatsapp.net"]
			const frendsowner = ["6281344309802@s.whatsapp.net"]
            ...
            if (!isUser) return reply(mess.only.userB)
                ...
                if (!isPremium) return reply(mess.only.premium)   
                  if (args.length < 1) return reply(`Masukan Teks\nContoh : ${prefix}bpink Ardyy Ganss`)
            ...
            case 'leveling':
                ...
                if (args[0] === '1') {
                    ...
                    reply(' *Ketik perintah 1 untuk mengaktifkan, 2 untuk menonaktifkan* \n *Contoh: ${prefix}leveling 1*')
                }
            break
					case 'cat':
					case 'kucing':
				...
				if (!isUser) return reply(mess.only.userB)
				if (!isPublic) return reply(mess.only.public)
					client.sendMessage(from, cat(prefix, sender), text)
					case 'ssweb':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (!isPublic) return reply(mess.only.public)
					...
					break
					case 'join':
            if (client.id == invitegroup) {
            if (args.length == 0) return client.reply(from, 'Bakajanaino?IZIN DULU KE > :081344309802', message.id)
            const link = body.slice(6)
            const minMem = 30
            const isLink = link.match(/(https:\/\/chat.whatsapp.com)/gi)
            const check = await client.inviteInfo(link)
            if (!isLink) return client.reply(from, 'Where\'s the link?IZIN DULU KE > :081344309802', message.id)
            ...
            await client.joinGroupViaLink(link).then( async () => {
                ...
                client.reply(from, 'Sumimasen 💔️IZIN DULU KE > :081344309802', message.id)
            })
            }
            break
                case 'anjing':
 ...
 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
			  	...
			  	if (!isUser) return reply(mess.only.userB)
		  		if (!isPublic) return reply(mess.only.public)
		    			client.sendMessage(from, ytbku(prefix, sender), text, {quoted: mek})
		      break
		   case 'bahasa':
		   if (isBanned) return reply(mess.only.benned)
		   if (!isUser) return reply(mess.only.userB)
		   ...
		   case '8bit':
                                        ...
                                        var tet1 = gh.split("/")[0];
                                        var tet2 = gh.split("/")[1];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}8bit Ardyy/Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                   if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=bit8&text1=${tet1}&text2=${tet2}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: '*ONIICHAN* Ini Gambarnya🤭...'})
                                        break
			case 'payment':
			  case 'pm':
			 if (!isUser) return reply(mess.only.userB)
			 if (!isPublic) return reply(mess.only.public)
			    client.sendMessage(from, payment(prefix, sender), text, {quoted: mek})
			    break
					case 'virtex':
...
conn.sendMessage(id, 'Halo Juga, Apakah Ada Yang Bisa Saya Bantu? Ketik .menu',MessageType.text, { quoted: m } );
}
  break
          case 'wa.me':
				  case 'wame':
  client.updatePresence(from, Presence.composing) 
    ...
    if (text.includes('P')){
conn.sendMessage(id, 'Pa Pe Pa Pe Usahakan Salam!',MessageType.text, { quoted: m } );
}
    client.sendMessage(from, options, text, { quoted: mek } )
				...
				break     
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					if (args.length < 1) return reply(mess.wrongf())
					aruga = body.slice(8)
					reply(mess.wait)
                ...
                break
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*APPOCALYPSE GROUP*\n\nLink : https://chat.whatsapp.com/JnATrE2rgDvLreybm6o35P', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
          ...
          if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (!isPublic) return reply(mess.only.public)
					client.sendMessage(from, nobg(prefix), text)
					break  
					case 'addfoto':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					...
					if (!isPublic) return reply(mess.only.public)
					if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply(`Fitur Nsfw Belum Dihidupkan Ketik ${prefix}nsfw 1 jika ingin mematikan ketik ${prefix}nsfw 2`)
			       ...
			       case 'valxlove':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin di jadiin user *PREMIUM*!')
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *.daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin di berhentiin jadi user *PREMIUM*!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT APPOCALYPSE ✨* 」\n*│+wa.me/6281344309802*╰──────*「 *APPOCALYPSE* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  APPOCALYPSE BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)
					 if (!isPublic) return reply(mess.only.public)
				...
				case 'addprem':
					...
					if (!isUser) return reply(mess.only.userB)
					if (args.length < 1) return reply(`Nomornya Mana Uey?\n*CONTOH* *:*${prefix}62813xxxxx@s.whatsapp.net`)
					if (!isOwner) return reply(mess.only.ownerB)
					var prmw = body.slice(9)
					var prm = prmw.split("/")[0];
						premium.push(prm)
						fs.writeFileSync('./src/premium.json', JSON.stringify(premium))
						client.sendMessage(from, `*[SUCCESS]*\nTelah Menambahkan Nomor ${prm.split("@s.whatsapp.net")[0]} Menjadi User Premium\n\n\nTotal User Premium ${premium.length}`, text, {quoted: mek})
					break
					case 'katakata':
					if (isBanned) return reply(mess.only.benned)
					 if (!isPublic) return reply(mess.only.public)
					...
					if (isBanned) return reply(mess.only.benned)
					if (!isPremium) return reply(mess.only.premium)
					if (args.length < 1) return reply(`Nomor Sama Pesannya Mana Om?\n*CONTOH* *:* ${prefix}send 628xxxx/Ardyy Ganss`)
					reply(mess.only)
					var pc = body.slice(6)
					var nomor = pc.split("/")[0];
					var org = pc.split("/")[1];
					reply(`*DONE!!* Berhasil Mengirim Pesan Ke Nomor ${nomor.split('@s.whatsapp.net')}`)
	...
	case 'spamsms':
					...
					var nomor = pc.split("/")[0];
					var org = pc.split("/")[1];
					reply(`*DONE!!* Berhasil Spam WA Ke Nomor ${nomor.split('@s.whatsapp.net')}`)
					client.sendMessage(nomor+'@s.whatsapp.net', org, text)
					break				
					case 'googleimage':
				   client.updatePresence(from, Presence.composing) 
				   ...
				   if (!isPublic) return reply(mess.only.public)
					...
					reply(`*「 MATH 」*\n\n*Kalkulator*\nHasil Dari ${mtk} = ${Math_js.evaluate(mtk)}`)
				}
				    break
					case 'chatdelete':
					if (isBanned) return reply(mess.only.benned)    
	...
	break
			...
			break
		case 'iri':
			client.sendMessage(from, 'sound' + 'baka.wav', audio, {quoted: mek, ptt:true})
			break
                case 'baka':
            case 'onichan':
            case 'bodoh':
                client.sendMessage(from, './lindy/baka.mp3', audio, {quoted: mek, ptt:true})
                break
				...
				if (!isPublic) return reply(mess.only.public)
					me = client.user
					uptime = process.uptime()
					teks = `*Nama bot* : ${me.name}\n*Author* : *Ardyy*\n*Nomor Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Public:* ON\n*Total User Premium*: ${premium.length}\n*Total Chat* : ${totalchat.length}\n*Instagram* : *ff.ardych*\n*Github* : *github.com/Appocalyp*\n*Youtube* : *Ardy FF (youtube.com/c/ArdyFF)*`
           ...
           break
                ...
                if (!isPremium) return reply(mess.only.premium)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, bufferlkj, audio, {mimetype: 'audio/mp4', quoted: mek, ptt:true})
						fs.unlinkSync(ran)
					})
					break
					case 'chatlist':
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USER BOT  APPOCALYPSE👑* 」\n\`\`\``
					no = 0
   ...
   case 'pubglogo':
                                        ...
                                        var tez1 = gh.split("/")[0];
                                        var tez2 = gh.split("/")[1];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}pubglogo Ardyy/Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=pubg&text1=${tez1}&text2=${tez2}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Logonya *ONICHAN*...'})
                                        break       
          ...
          contextInfo: { mentionedJid: [groupOwner] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
	          ...
	          if (!isUser) return reply(mess.only.userB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA SU*`
 ...
 if (!isOwner) return reply(mess.only.ownerB)
                ...
                case 'qm':
                    ...
                    tes2 = gh.split("/")[1];
                    tes3 = gh.split("/")[2]
                    data = await fetchJson(`https://terhambar.com/aw/qts/?kata=${tes1}&author=${tes2}&tipe=${tes3}`)
		...
		case 'woodblock':
                                        var gh = body.slice(10)
                                        var ted1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}woodblock Ardy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.onyl.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=wood_block&text=${ted1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: '*ONIICHAN* Ini gambarnya🤭...'})
                                        break
				case 'minion':
                ...
                case 'lionlogo':
                                        ...
                                        var teg1 = gh.split("/")[0];
                                        var teg2 = gh.split("/")[1];
                                        if (args.length < 1) return reply(`teksnya mana um\n*Contoh:* ${prefix}lionlogo Ardyy|Ganss`)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=lionlogo&text1=${teg1}&text2=${teg2}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
	...
	if (!isPublic) return reply(mess.only.public)
				...
				if (!isUser) return reply(mess.only.userB)
				if (!isPublic) return reply(mess.only.public)
					if (args.length < 1) return client.sendMessage(from, `Kode bahasanya mana om?\nKetik ${prefix}bahasa Untuk Melihat Kode Bahasa`, text, {quoted: mek})
	...
	if (!isPublic) return reply(mess.only.public)
				...
				if (!isPublic) return reply(mess.only.public)
					watak = body.slice(1)
					const wa =['penyayang','pemurah','Pemarah','Pemaaf','Penurut','Baik','baperan','Baik Hati','penyabar','Uwu','top deh, pokoknya','SukaMembantu']
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6281344309802@s.whatsapp.net', options, text, {quoted: mek})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					break
				case 'spamsms':
					if (args.length < 1) return reply('𝓝𝓸𝓶𝓸𝓻 𝓣𝓪𝓻𝓰𝓮𝓽𝓷𝔂𝓪 𝓜𝓪𝓷𝓪 𝓒𝓾𝔂?')
					tels = body.slice(9)
					reply(mess.wait)
			...
			if (!isPublic) return reply(mess.only.public)
				...
				if (!isUser) return reply(mess.only.userB)
					reply(mess.wait)
					anu = await fetchJson(`https://api.i-tech.id/tools/cat?key=Kr3XuC-GCDdIh-trhtiC-1GUwLx-UGNkxG`)
					if (anu.error) return reply(anu.error)
                ...
                reply(mess.wait)
					...
					client.sendMessage(from, bufferyyy, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek, ptt:true})
					break
			    case 'bucin':
 ...
 if (isBanned) return reply(mess.only.benned)    
				...
				break
					...
					case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teqs1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}jokerlogo Ardyy Ganss`)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=jokerlogo&text=${teqs1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Logonya *ONICHAN*...'})
                                        break
					case 'ranime':
          ...
          reply(mess.wait)
                                       ...
                                       if (args[0].startsWith('83816252036')) return reply('Gagal tidak dapat menelpon nomer bot')
                                       if (args[0].startsWith('81344309802')) return reply('Gagal tidak dapat menelpon nomer owner')
                                       var data = body.slice(10)
                                       reply(`*[SUCCESS]* Berhasil Spam Ke Nomor HP 62`+data, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
                 if (!isUser) return reply(mess.only.userB)
                 if (args.length < 1) return reply(`Masukan Nomor\nContoh : ${prefix}infonomor 0813xxxx`)
			...
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (data.error) return reply(data.error)
                ...
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.title}.mp3`, quoted: mek, ptt:true})
                break
                case 'slap':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
if (isBanned) return reply(mess.only.benned)    
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            imgbb('727e7e43f6cda1dfb85d888522fd4ce1', media)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					case 'burnpaper':
                                        var gh = body.slice(10)
                                        var tex1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}burnpaper Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=burn_paper&text=${tex1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: '*ONIICHAN* Ini Gambarnya🤭...'})
                                        break
				case 'afk':
                       ...
                       if (!isPublic) return reply(mess.only.public)
                                        var nom = mek.participant
                                        const tg = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *SEDANG AFK ${tels} JANGAN GANGGU YE SU\n*BAKA!!**`,
                                                contextInfo: { mentionedJid: [nom] }
                                        }
                                        client.sendMessage(from, tg, text, {quoted: mek})
                                        break	
				case 'nulis': 
				case 'tulis':
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  APPOCALYPSE BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * APPOCALYPSE Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
			...
			if (!isPublic) return reply(mess.only.public)
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Reply pesan yang mau di hapus!\n*BAKAA!!*')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
			...
			if (!isPublic) return reply(mess.only.public)
				...
				if (!isPublic) return reply(mess.only.public)
					...
					case 'coffee':
                                        var gh = body.slice(7)
                                        var tew1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}coffee Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=coffee&text=${tew1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var tee1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}lovepaper Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=love_paper&text=${tee1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var ter1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}woodblock Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=wood_block&text=${ter1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var tey1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}qowheart Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=quote_on_wood_heart&text=${tey1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teu1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}mutgrass Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=message_under_the_grass&text=${teu1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
                                case 'undergocean':
                                        ...
                                        var tep1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}undergocean Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=underwater_ocean&text=${tep1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
                                case 'woodenboards':
                                        ...
                                        var teb1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}woodenboards Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=wooden_boards&text=${teb1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
          case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`Teksnya mana Om?\n*Contoh:* ${prefix}harrypotter Ardyy Ganss`)
					...
					case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teq1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}wolfmetal Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=wolf_metal&text=${teq1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
					case 'metalictglow':
                                        ...
                                        var tea1 = gh.split("/")[0];
                                        if (args.length < 1) return reply(`Teksnya Mana Om?\n*Contoh:* ${prefix}metalictglow Ardyy Ganss`)
                                        if (!isUser) return reply(mess.only.userB)
                                        if (!isPublic) return reply(mess.only.public)
                                        if (!isPremium) return reply(mess.only.premium)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=metalic_text_glow&text=${tea1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini Gambarnya *ONICHAN*...'})
                                        break
				case 'toimg':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 untuk mengaktifkan, 2 untuk menonaktifkan')
					}
                                      break
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
...
conn.sendMessage(id ,`${gg} ${exists ? " exists " : " does not exist"} on WhatsApp`, MessageType.text)
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('HANYA OWNER YANG DAPAT MENGGUNAKAN NYA\n*BAKA!!*')
					if (args.length < 1) return reply('Hhhmmm...')
					if (Number(args[0]) === 1) {
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				case `addadmin`:
                   ...
                   adminNumber.push(mentionedJid[i])
                   fs.writeFileSync('./src/admin.json', JSON.stringify(adminNumber))
